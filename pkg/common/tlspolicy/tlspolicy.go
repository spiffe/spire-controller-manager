...
func LogPolicy(policy Policy, logger logr.Logger) {
	if policy.RequirePQKEM {
		logger.Info("Experimental option 'require_pq_kem' is enabled; webhook TLS connections will require use of a post-quantum safe KEM")
	}
}
